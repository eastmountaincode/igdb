...
The accepted reliability profile is 720px, 4px cells, four data symbols protected by Hamming(7,4) on both bit planes, two temporal repeats, compact Reed-Solomon-style 64+12 parity headers, 6 Mbps H.264 with constant bitrate and realtime latency mode, and two-way segment concurrency. The browser renders every canvas explicitly in sRGB. Before upload, Zo materializes the browser's three-frame holds as constant 30 fps video, preserves the browser-generated pixel values, and tags every data video as SMPTE 170M so browser-dependent color metadata cannot change Instagram's pipeline. The resulting stream carries fifteen Hamming-protected logical chunks per second while preserving two physical copies. The app must decode, reassemble, and SHA-256 verify these server-normalized staged videos before enqueueing a public Instagram post. Symbol conversion uses independent 8-byte/25-symbol radix blocks rather than one grid-sized `BigInt`; this is the primary encoding-speed optimization. Browsers without CBR support fall back to the separately Instagram-verified realtime VBR mode. The 64+12 parity layout repairs up to twelve missing data frames in one group and remains backward-compatible with older XOR and 64+8 posts. Data frames are physically interleaved across parity groups, and the first eight transmitted data frames are duplicated later in every video to survive Instagram startup damage; the 60-second target preserves a conservative 14 MB source limit across seven data videos plus one display video. Every file byte is stored in the visual channel. Each video also carries up to five independently synchronized 16-byte DTMF packets containing a redundant copy of real file bytes, so Instagram audio damage can never make an otherwise complete file unrecoverable. The browser sends those copied bytes beside the silent MP4, and the Zo publisher generates mono PCM, encodes it as AAC with ffmpeg, and muxes it before Meta fetches the staged video. The robust DTMF alphabet uses four tone pairs and base-4 symbols because the full 16-tone alphabet did not preserve arbitrary bytes through AAC. Decoding remains backward-compatible with older six-color/three-repeat posts, older posts that stored a unique tail in audio, the older whole-grid symbol conversion, and array-based or compact parity headers. Parity recovery must derive payload capacity from the decoded frame format rather than the current encoder constants; this is required to reconstruct missing chunks from legacy posts.

Codec compatibility is append-only. `src/codec-registry.ts` names every supported visual format and exposes a decoder revision through `/api/codec-capabilities`. Add future encoders as new registry entries without deleting or rewriting existing decoders. The archive records the decoder revision and detected codec for each recovery; a failed post is retried once when the registry revision changes, while transient network/download failures remain retryable every run. Never offer a recovered download unless final SHA-256 verification passes. Do not weaken the codec based on local decoding alone; every codec change must survive an Instagram publish/download round trip and final SHA-256 verification.
...
Every Write creates an eight-second, 720×720 H.264 **display video** containing the source filename, MIME type, and size. It is always carousel item 1 before the encoded data videos. An optional animated GIF may appear inside its padded safe area. The UI previews the actual generated MP4, not a responsive HTML imitation. The Hamming source limit is 14 MB so seven data videos plus the display video stay within Instagram's eight-item limit. Read must continue accepting and reconstructing older files up to the previous 25 MB limit. Published media IDs and their display-video flag are recorded in the runtime-only `.instagram-media-index.json`, allowing Read to omit item 1 before decoding.
